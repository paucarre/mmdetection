...
#
# INSTALL 'mmdetection' ENVIRONMENT WITH ALL DEPENDENCIES
#
ENVS=$(conda env list | awk '{print $1}' )
if [[ $ENVS = *mmdetection* ]]; then
   echo -e "Anaconda 'mmdetection' environment detected."
   ...
   conda deactivate
   conda activate mmdetection
   echo "Run 'conda activate mmdetection' to enable python virtual environment to run the scripts."
fi;
